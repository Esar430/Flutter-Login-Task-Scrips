import 'dart:async';
import 'dart:io';
import 'package:bloc_login/data/constants/shared_preference.dart';
...
class AuthBloc extends HydratedBloc<AuthEvent, AuthState> {
  AuthBloc({AuthenticationRepository authenticationRepository})
    ...
    on<AuthLoginRequested>(_onAuthenticationSharedPrefsLoginRequested);
  }
  ...
  Future<FutureOr<void>> _onAuthenticationInit(
    ...
    emit(state is AuthGrantedSharePrefs ? state : AuthInitial());
  }
  ...
  FutureOr<void> _onAuthenticated(
    ...
    emit(const AuthGrantedSharePrefs());
  }

  Future<FutureOr<void>> _onAuthenticationLoginRefresh(
      AuthRefreshRequested event, Emitter<AuthState> emit) async {}

  @override
  AuthState fromJson(Map<String, dynamic> json) {
    try {
      return const AuthGrantedSharePrefs();
    } catch (_) {
      return AuthInitial();
    }
  }
  ...
  FutureOr<void> _onAuthenticationSharedPrefsLoginRequested(
      AuthLoginRequested event, Emitter<AuthState> emit) {
    emit(AuthLoading());

    if(event.email == Prefs.getString(Prefs.myMail) &&
        event.password == Prefs.getString(Prefs.myPassword)){
      emit(const AuthGrantedSharePrefs());
    }else{
